...
// Notes:
//   - Markers + routes use constant colour (GREEN for Commerce)
//   - FIX: Routes only come from the FIRST location (origin)
//   - FIX: Avoid stacked dots at identical coordinates (origins + destinations)
//   - Locations list in right panel can include all locations; hover tooltip stays per-marker label
// ==============================
...
function addInteractiveMarker(obj, lat, lng, locLabel) {
  ...
  // Ensure markers fade back to the BASE opacity
...
function drawForObject(objectId) {
  ...
  // ✅ One marker per coordinate globally (prevents stacked dots)
  const seenMarkers = new Set();

  function addMarkerOnce(lat, lng, label) {
    const key = `${Number(lat).toFixed(6)},${Number(lng).toFixed(6)}`;
    if (seenMarkers.has(key)) return null;
    seenMarkers.add(key);
    return addInteractiveMarker(obj, lat, lng, label);
  }

  // Add origin marker
  addMarkerOnce(origin.lat, origin.lng, origin.label);

  // Add other locations as markers (panel list already includes them)
  ...
  for (const r of routes) {
    ...
    // Destination marker (one per coordinate)
    addMarkerOnce(to.lat, to.lng, r.toLabel || "Destination");
